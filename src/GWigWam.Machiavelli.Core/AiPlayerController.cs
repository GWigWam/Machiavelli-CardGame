...
public class AiPlayerController(Game game, Player player) : PlayerController
{
    ...
    {
        var rght = PickInfo.LeftHand.Length == 0 || (PickInfo.RightHand.Length > 0 && PickInfo.RightHand.Average(p => p.Gold) >= PickInfo.LeftHand.Average(p => p.Gold)); // Steal from side with highest avg gold
        var possibleChars = rght ? PickInfo.PossibleRightCharacters : [.. PickInfo.Available.Where(c => c.Type != CharacterType.Known.Thief)];
        var rich = (rght ? PickInfo.RightHand : PickInfo.LeftHand).OrderBy(p => p.Gold).ThenBy(_ => Random.Shared.NextDouble()).Last(); // Attempt to steal from richest player on that side
